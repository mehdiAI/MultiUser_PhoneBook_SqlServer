...
BEGIN
        ...
        --SELECT * from names , phoneNumber where id in (select id from @nid) AND id = name_id
        /*
        SELECT * from names , phoneNumber , email , Address  where 
                                                                id in (select n_id from @nid) AND 
                                                                phoneN_id in (select p_id from @pid) AND 
                                                                email_id in (select e_id from @eid) AND 
                                                                Address_id in (select a_id from @aid) AND
                                                                id = phoneNumber.name_id AND id = email.name_id AND id = Address.name_id;
*/
        select * from names 
...
--BEGIN
        --BEGIN TRY
        --SELECT * from names where first_name LIKE '%'+@fname+'%' AND last_name LIKE '%'+@lname+'%';
